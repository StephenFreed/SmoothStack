...
try:
    with open("SmoothStack/Assignment_5/input_data.txt", "r") as input_file:
    ...
    print("\nThere was a problem reading from input_data.txt.")
...
try:
    with open("SmoothStack/Assignment_5/bmi_results.txt", "w") as answer_file:
        answer_file.write("These are the BMI results...\n")
        for row in range(num_of_people):  # type: ignore
            answer_row = f"Person {row+1} BMI Result: {bmi_results[row]}\n"
            answer_file.write(answer_row)
except:  # noqa
    print("\nThere was a problem writing results to bmi_results.txt")
